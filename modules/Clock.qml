...
Item {
    ...
    // Item) instead of a plain child — the calendar grid is ~130 QQuickItems
    // behind a popup opened maybe a few times a session, and each of those
    // real windows keeps its own GPU context alive for the life of the
    // process once created (measured: ~3-4MB RSS per popup window, never
    // freed on hide). Destroying it on close via active = false trades that
    // permanent cost for a one-frame recreation delay next time it's hovered.
    LazyLoader {
        id: popupLoader
        active: false
        ...
        HoverPopup {
            ...
            // Month currently displayed, independent of the live clock —
            // reset to the current month each time the popup opens so
            // navigating away never leaves it stranded on an old month next
            // time it's shown.
            ...
            // viewMonth only while visible, so resetting them here — before
            // the next open, rather than racing the same visibleChanged
            // signal that flips 'visible' true — guarantees the grid opens
            // on the current month rather than wherever navigation last
            // left it.
            //
            // Also tears the popup down once the close grace period elapses
            // (this fires after HoverPopup's own hideTimer, so it's safe —
            // nothing is still relying on the popup being open at this
            // point).
            onVisibleChanged: {
                if (!visible) {
                    goToday();
            ...
            // Qt::DayOfWeek (Monday=1 .. Sunday=7) -> JS Date.getDay()
            // convention (Sunday=0 .. Saturday=6), so locale and calendar
            // math share one frame of reference throughout.
            ...
            // 42 cells (6 weeks) covering the leading/trailing days of
